...
QUnit.test('Test: Assessment Result Program', function(assert){
	assert.expect(25);
	...
	frappe.run_serially([
		// Saving Assessment Plan Program name
		() => frappe.db.get_value('Assessment Plan Program', {'assessment_name': 'Test-Mid-Term'}, 'name'),
		...
		() => frappe.set_route('Form', 'Assessment Plan Program', assessment_name),
		() => frappe.timeout(1),
		() => frappe.tests.click_button('Assessment Result Program'),
		() => frappe.timeout(1),
		() => cur_frm.refresh(),
		() => frappe.timeout(1),
		() => {
		...
		() => {
			student_list.forEach(index => {
				tasks.push(
					() => frappe.set_route('List', 'Assessment Result Program', 'List'),
					() => frappe.timeout(0.5),
					...
					() => frappe.db.get_value('Assessment Plan Program', {'name': 'ASP00001'}, ['grading_scale', 'maximum_assessment_score']),
					(assessment_plan) => {
