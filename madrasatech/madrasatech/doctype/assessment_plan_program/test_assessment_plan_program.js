...
QUnit.test('Test: Assessment Plan Program', function(assert){
	assert.expect(6);
	...
	frappe.run_serially([
		...
		() => {
			return frappe.tests.make('Assessment Plan Program', [
		...
		() => frappe.click_button('Assessment Result Program'), // Checking out Assessment Result button option
		() => frappe.timeout(0.5),
		() => {
			assert.deepEqual(frappe.get_route(), ["Form", "Assessment Result Program Tool"], 'Assessment Result properly linked');
