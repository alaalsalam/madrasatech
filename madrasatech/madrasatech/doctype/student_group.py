...
class StudentGroup(Document):
	...
	def validate_students(self):
		program_enrollment = get_program_enrollment_2(
			self.academic_year,
			self.academic_term,
			self.program,
			self.batch,
...
def get_students_2(
	...
	enrolled_students = get_program_enrollment_2(
	...
	if enrolled_students:
		...
		return student_list[int(first_student_number) : int(first_student_number) + int(second_student_number)]
	else:
		frappe.msgprint(_("No students found"))
		return []


def get_program_enrollment_2(
...
@frappe.whitelist()
def update_number_of_students_in_program(program, index_of_students_in_groups, max_strength):
    doc = frappe.get_doc('Program', program)
    doc.index_of_students_in_groups = int(index_of_students_in_groups) + int(max_strength)
    doc.students_out_of_groups = int(doc.total_number_of_students) - (int(index_of_students_in_groups) + int(max_strength))
    doc.save()
