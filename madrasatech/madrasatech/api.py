...
def enroll_student(source_name):
    ...
    """
    frappe.publish_realtime("enroll_student_progress", {
                            "progress": [1, 4]}, user=frappe.session.user)
    student = get_mapped_doc(
        "Student Applicant",
        source_name,
        {"Student Applicant": {"doctype": "Student",
    ...
    frappe.publish_realtime("enroll_student_progress", {
                            "progress": [2, 4]}, user=frappe.session.user)
...
def get_student_guardians(student):
    ...
    :param student: Student.
    """
    guardians = frappe.get_all("Student Guardian", fields=[
                               "guardian"], filters={"parent": student})
    return guardians
        ...
        if result:
            ...
            for d in result.details:
                student_result.update(
                    {d.assessment_criteria: [cstr(d.score), d.grade]})
            student_result.update(
                {"total_score": [cstr(result.total_score),
                                 result.grade], "comment": result.comment}
            )
            student.update(
                {"assessment_details": student_result,
                    "docstatus": result.docstatus, "name": result.name}
            )
        else:
...
def mark_assessment_result(assessment_plan, scores):
    ...
    for criteria in student_score.get("assessment_details"):
        assessment_details.append(
            {"assessment_criteria": criteria, "score": flt(
                student_score["assessment_details"][criteria])}
        )
    ...
    for stud in students:
        for guard in get_student_guardians(stud.student):
            email = frappe.db.get_value(
                "Guardian", guard.guardian, "email_address")
            if email:
...
@frappe.whitelist()
def get_assessment_details_program(assessment_plan):
...
@frappe.whitelist()
def mark_assessment_result_program(assessment_plan, scores):
    student_score = json.loads(scores)
    assessment_details = []
    for criteria in student_score.get("assessment_details"):
        assessment_details.append(
...
def get_assessment_result_doc_program(student, assessment_plan):
    assessment_result = frappe.get_all(
    ...
    if assessment_result:
        doc = frappe.get_doc("Assessment Result Program", assessment_result[0])
        if doc.docstatus == 0:
            return doc
        elif doc.docstatus == 1:
            frappe.msgprint(_("Result already Submitted"))
        ...
        return frappe.new_doc("Assessment Result Program")


@frappe.whitelist()
def submit_assessment_results_program(assessment_plan, student_group):
    total_result = 0
    student_list = get_student_group_students(student_group)
    for i, student in enumerate(student_list):
        doc = get_result_program(student.student, assessment_plan)
        if doc and doc.docstatus == 0:
            total_result += 1
            doc.submit()
    return total_result
...
@frappe.whitelist()
def get_result_program(student, assessment_plan):
    """Returns Submitted Result of given student for specified Assessment Plan
...
@frappe.whitelist()
def get_assessment_students_program(assessment_plan, student_group):
    student_list = get_student_group_students(student_group)
    for i, student in enumerate(student_list):
...
@frappe.whitelist()
def get_assessment_students_program_all_coures(assessment_plan, assessment_criteria_program, student_group, type_test,assessment_group,academic_term):
    student_list = get_student_group_students(student_group)
    for student in student_list:
        result = get_result_program_all_coures(
            student.student, assessment_criteria_program, type_test,assessment_group,academic_term)
        if result:
            student_result = {}
            for d in result:
                student_result.update(
...
@frappe.whitelist()
def get_result_program_all_coures(student, assessment_criteria_program, type_test,assessment_group,academic_term):
    """Returns Submitted Result of given student for specified Assessment Plan
